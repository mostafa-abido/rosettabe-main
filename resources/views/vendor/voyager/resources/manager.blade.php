...
<div>
                ...
                <div class="file_link selected" aria-hidden="true" data-toggle="tooltip" data-placement="auto" :title="file">
                    <div class="link_icon">
                        <template v-if="fileIs(file, 'image')">
                            <div class="img_icon" :style="imgIcon(current_folder+file)"></div>
                        </template>
                        ...
                        <template v-else-if="fileIs(file, 'link')">
                            <i class="icon voyager-folder"></i>
                        </template>
                        <template v-else>
    ...
    <div id="toolbar" v-if="showToolbar" :style="isExpanded ? 'display:block' : 'display:none'">
        ...
        <div class="btn-group offset-right">
          <button type="button" class="btn btn-primary" id="createText" data-toggle="modal" :data-target="'#create_text_modal_'+this._uid">
            <i class="icon voyager-forward"></i>
            Add Link
          </button>
        </div>
                    ...
                    <li v-for="(file) in files" v-on:click="selectFile(file, $event)" v-on:dblclick="openFile(file)" v-if="filter(file)">
                        <div :class="'file_link ' + (isFileSelected(file) ? 'selected' : '')">
                            ...
                            <div class="details">
                                <div :class="file.type">
                                    <h4>@{{ file.name }}</h4>
                                    <small v-if="!fileIs(file, 'folder') && !fileIs(file, 'link')">
                ...
                <div class="right_details">
                    ...
                    <div v-else-if="selected_files.length == 1" class="right_details">
                        ...
                        <div class="detail_info">
                            ...
                            <template v-if="!fileIs(selected_file, 'folder') && !fileIs(selected_file, 'link')">
                                <span>
                            ...
                            <span v-if="fileIs(selected_file, 'link')">
                              <h4>URL</h4>
                              <p><a :href="selected_file.content" target="_blank">{{ __('voyager::generic.click_here') }}</a></p>
                            </span>
    ...
    <!-- New Text Modal -->
    <div class="modal fade modal-info" :id="'create_text_modal_'+this._uid">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title">@{{ this.modals.text.id ? 'Update' : 'Create' }} New Text/Link</h4>
                </div>

                <div class="modal-body">

                    <input name="textTitle" placeholder="Title" class="form-control" value="" />

                    <br>

                    <input name="newUrl" placeholder="Link" class="form-control" value="" />
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('voyager::generic.cancel') }}</button>
                    <button type="button" class="btn btn-info" v-on:click="createText">@{{ this.modals.text.id ? 'Update' : 'Create' }}
                    </button>
                </div>
            </div>
        </div>
    ...
    Vue.component('media-manager-2', {
        ...
        props: {
            ...
            details: {
                type: Object,
                default: function() {
                    return { show_folders: true, show_as_images: true };
                }
            },
            expanded: {
                type: Boolean,
        ...
        methods: {
            getFiles: function() {
                var vm = this;
                vm.is_loading = true;
                $.post('{{ route('filemanager.files') }}', { folder: vm.current_folder, _token: '{{ csrf_token() }}', details: vm.details }, function(data) {
                    vm.files = [];
            ...
            openFile: function(file) {
                if (file.type == 'folder') {
                    ...
                    } else if (this.fileIs(this.selected_file, 'link')){
                        window.open( '_blank')
                    }
                }
            },
            ...
            fileIs: function(file, type) {
                if (typeof file === 'string') {
                    ...
                    if (type == 'text') {
                      return this.endsWithAny(['txt'], file.toLowerCase());
                    }
                    //Todo: add other types
                } else {
                  if (file.type.includes(type) || !file.extension) {
                    return file.type.includes(type);
                  } else {
                    if (type == 'image') {
                        return this.endsWithAny(['jpg', 'jpeg', 'png', 'bmp'], file.extension.toLowerCase());
                    }
                    if (type == 'link') {
                      return this.endsWithAny(['url'], file.extension.toLowerCase());
                    }
                    if (type == 'text') {
                      return this.endsWithAny(['txt'], file.extension.toLowerCase());
                    }
                  }
                }
            ...
            createFolder: function(e) {
                ...
                $.post('{{ route('filemanager.new_folder') }}', { new_folder: vm.current_folder+'/'+name, _token: '{{ csrf_token() }}' }, function(data) {
					if(data.success == true){
            ...
            createText: function(e) {
              var vm = this,
                  link = $('input[name=newUrl]').val(),
                  title = $('input[name=textTitle]').val();

                  const file = new File([link], `${title}.url`)
                  ...
                  $('#create_text_modal_'+vm._uid).modal('hide');
                  ...
                  $.ajax({
                    ...
                    success: (data) => {
                      ...
                      console.log(data);
                      if(data.success == true){
                        toastr.success('Link created', "{{ __('voyager::generic.sweet_success') }}");
                        vm.getFiles();
                      } else {
                        toastr.error(data.error, "{{ __('voyager::generic.whoopsie') }}");
                      }
            ...
            crop: function(mode) {
				...
				$.post('{{ route('filemanager.crop') }}', postData, function(data) {
					if (data.success) {
        ...
        mounted: function() {
            ...
            $(document).ready(function () {
                ...
                $('#create_text_modal_' + vm._uid).on('hidden.bs.modal', function () {
                    tinyMCE.get('newText').setContent('');
                    $('input[name=textTitle]').val('');
                    vm.modals.text.id = null;
                });
                ...
                var additionalConfig = {selector: 'textarea.richTextBox',min_height: "100"};
