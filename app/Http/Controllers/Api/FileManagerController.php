...
class FileManagerController extends Controller
{
  ...
  {
    $this->filesystem = config('voyager.storage.aws-s3-resources');
  }
  ...
  public function index()
  {
    ...
    return view('vendor.voyager.resources.index');
  }
  ...
  public function files(Request $request)
  {
    ...
    $storageItems = $storage->listContents($dir, false);

    $files = [];

    foreach ($storageItems as $item) {
      if ($item['type'] == 'dir') {
        $items = Storage::disk($this->filesystem)->allFiles($item['path']);

        foreach ($items as $key => $it) {
          $items[$key] = Storage::disk($this->filesystem)->url($it);
        }
        ...
        if (isset($item['extension'])) {
          if ($item['extension'] === 'url') {
            $file['content'] = Storage::disk($this->filesystem)->get($item['path']);
  ...
  public function move(Request $request)
  {
    ...
    foreach ($request->get('files') as $file) {
      ...
      if ($file['type'] === 'link') {
        $old_path = $path . $file['name'] . '.url';
        $new_path = $dest . $file['name'] . '.url';
      }
  ...
  public function rename(Request $request)
  {
    ...
    $fullFilePathNew = "{$location}/{$newFilename}";

    if (!Storage::disk($this->filesystem)->exists($fullFilePathNew)) {

      if (Storage::disk($this->filesystem)->move($fullFilePath, $fullFilePathNew)) {
        $success = true;
      } else {
        ...
        if (!isset($filePathInfo['extension'])) {
          Storage::disk($this->filesystem)->makeDirectory($fullFilePathNew);

          $files = Storage::disk($this->filesystem)->allFiles($fullFilePath);

          if ($files) {
            foreach ($files as $file) {
              $new_loc = str_replace($filename, $newFilename, $file);

              Storage::disk($this->filesystem)->copy($file, $new_loc);
            }
          }

          Storage::disk($this->filesystem)->deleteDirectory($fullFilePath);
