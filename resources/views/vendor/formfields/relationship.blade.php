        ...
        @elseif($options->type == 'belongsToMany')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @php
                    ...
                    $mapped_values = $selected_values->map(function ($item, $key) use ($options) {
            			    return $item->{$options->label};
                    })->all();
                @endphp
                ...
                @if($view == 'browse')
                    @php
                        $string_values = implode(", ", $mapped_values);
                    @endphp
                ...
                @else
                    ...
                    @else
                        <ul>
                            @foreach($mapped_values as $selected_value)
                                <li><a href="/admin/users/{{$selected_values->firstWhere($options->label, $selected_value )->id}}">{{ $selected_value }}</a></li>
                            @endforeach
                        </ul>
                    @endif
                @endif
